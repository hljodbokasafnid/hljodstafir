...
def get_seconds(time_string):
  if ('s' in time_string):
    return '{0:.3f}'.format(float(time_string.replace('s','')))
...
def process_smil_files(foldername):
  ...
  if not rerun:
    ...
    with open('././public/output/{}/ncc.xhtml'.format(foldername), 'r', encoding='utf8') as f:
      ncc = f.read()
    ...
    for index, smil_file in enumerate(smil_files):
      ...
      if smil_tag:
        ...
        for audio_index, audio_tag in enumerate(audio_tags):
          if audio_tag['clipEnd'] != "0s" and audio_tag['clipBegin'] != "0s":
        ...
        new_head.insert(0, meta_generator) if meta_generator else None
        new_head.insert(0, meta_time_in_smil) if meta_time_in_smil else None
        new_head.insert(0, meta_total_elapsed_time) if meta_total_elapsed_time else None
        new_head.insert(0, meta_identifier) if meta_identifier else None
        new_head.insert(0, meta_format) if meta_format else None
        new_head.insert(0, meta_title) if meta_title else None
